---
title: "docker 환경 설정하기"
description: |
  docker 기반으로 Shiny 서버를 운영하기 위한 환경설정을 수행합니다.
...
## docker 환경 설정하기
우리는 Shiny 데모 애플리케이션을 실행하기 위해서 Shiny 서버를 설치하지는 않을 것입니다. Shiny 데모 애플리케이션이 구동되는 최소 환경은 "아 Shiny 애플리케이션이 서버에서 실행되는 구나" 정도를 인지할 뿐, 사용자가 개발한 애플리케이션을 운영하기 위해서는 몇가지 작업을 수행해야 합니다.  

우리는 **오픈통계 패키지인 BitStat를 Shiny 서버에서 운영하는 것을 전제로 Shiny 서버의 docker 환경을 설정**할 것입니다.

docker를 업무에, 혹은 자주 사용하지 않는 관계로 docker에 정통하지는 않습니다. 아주 기본적인 지식만 가지고 있을 뿐이죠. 그래서 개인적으로 Mac OS에서 **Docker Desktop**라는 Docker 관리 애플리케이션을 사용하고 있습니다. 
...
Shiny 서버는 3838 포트를 사용합니다. 여기서는 Shiny 서버의 기본 포트인 3838을 사용합니다. 
...
docker run -d -p 3838:3838 \
    -v /Users/choonghyunryu/web/html/shiny/:/srv/shiny-server/ \
    -v /Users/choonghyunryu/Documents/99_logos/shiny/:/var/log/shiny-server/ \
    rocker/shiny
```
...
로컬 호스트의 /Users/choonghyunryu/web/html/shiny/ 디렉토리에는 서브 디렉토리로 BitStat 애플리케이션이 위치하기 때문입니다.
...
에러가 발생한 원인을 호스트의 로그 볼륨에서 확인할 수 있습니다. 호스트의 /Users/choonghyunryu/Documents/99_logos/shiny 디렉토리에 있는 로그 파일을 열어봅니다.
